            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(settings.DISCORD_WEBHOOK_URL, json=message) as response:
                    if response.status != 204:
                        ...
                        self.get_logger("DiscordHandler").error(f"Failed to send Discord message: Status {response.status}, Response: {response_text}")
        except aiohttp.ClientError as e:
            self.get_logger("DiscordHandler").error(f"Aiohttp client error sending Discord message: {e}")
        except Exception as e:
            self.get_logger("DiscordHandler").error(f"Unexpected error in _send_discord_message_async: {e}")
...
DiscordHandler.get_logger("BinanceBot").addHandler(discord_handler)