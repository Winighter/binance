...
class FuturesTradingManager:
    ...
    def calculate_quantity_with_risk_management(
        ...
        risk_percentage: Decimal = app_config.MAX_RISK_RATIO,
    ) -> Decimal:
        """
        ...
        return Decimal('0') # 수량이 0보다 작거나 symbol 정보가 없을 경우 0 반환
    ...
    def create_take_profit_order(self, symbol: str, side: str, quantity: Decimal, stop_price: Decimal, positionSide: str) -> Optional[dict]:
        return self._create_market_order_with_stop_price(symbol, side, 'TAKE_PROFIT_MARKET', quantity, stop_price, positionSide)