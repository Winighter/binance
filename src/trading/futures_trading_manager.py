...
class FuturesTradingManager:
    ...
    def calculate_quantity_with_risk_management(
        ...
        leverage: Decimal = Decimal('1'),  # 레버리지를 입력 변수로 추가
        symbol: str = None
    ) -> Decimal:
        """
        ...
        max_loss_amount = balance_usdt * risk_percentage
        ...
        # 3. 총 포지션 가치 계산 (레버리지 미적용)
        total_position_value = max_loss_amount / loss_per_unit
        
        # 4. 레버리지 적용 후 실제 매수 수량 계산
        quantity = (total_position_value / price) * leverage
