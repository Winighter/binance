...
class BinanceSetupManager:
    ...
    def _setup_initial_state(self) -> tuple[List[Decimal], List[Decimal], List[Decimal], List[Decimal], Decimal]:
        ...
        try:
            all_usdt, available_usdt = self._fetch_balance()
            self._fetch_symbol_info()
            self._fetch_initial_klines()
            self._fetch_positions()
            self._setup_trading_environment()
            return self.open_prices, self.high_prices, self.low_prices, self.close_prices, self.volume_prices, all_usdt, available_usdt, self.leverage
    ...
    def _fetch_balance(self) -> Decimal:
        ...
        if account_balance == []:
            return logger.warning(f"Balance not found. This message is not error, but it may cause problems later.")
        for balance in account_balance:
            if balance.get('asset') == 'USDT':
                ...
                return self.balance_usdt, available_usdt
