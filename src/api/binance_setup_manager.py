...
class BinanceSetupManager:
    ...
    def _setup_initial_state(self) -> tuple[List[Decimal], List[Decimal], List[Decimal], List[Decimal], Decimal]:
        ...
        try:
            assets = self._fetch_balance()
            self._fetch_symbol_info()
            self._fetch_initial_klines()
            self._fetch_positions()
            self._setup_trading_environment()
            return [self.open_prices, self.high_prices, self.low_prices, self.close_prices, self.volumes], assets, self.leverage
    ...
    def _fetch_balance(self) -> List | None:
        ...
        try:
            for asset in account_balance:
                stable_coin = asset.get('asset')
                balance = Decimal(str(asset.get('balance')))
                if balance > Decimal('0'):
                    crossWalletBalance = Decimal(asset.get('crossWalletBalance'))
                    crossUnPnl = Decimal(asset.get('crossUnPnl'))
                    availableBalance = Decimal(asset.get('availableBalance'))
                    maxWithdrawAmount = Decimal(asset.get('maxWithdrawAmount'))
                    assets.append({stable_coin:{'balance':balance,'crossWalletBalance': crossWalletBalance, 'crossUnPnl': crossUnPnl, 'availableBalance': availableBalance, 'maxWithdrawAmount': maxWithdrawAmount}})
            return assets
        except Exception as e:
    ...
    def _fetch_positions(self):
        ...
        if not positions:
            return logger.info("POSITION: No open positions found.")
    ...
    def _fetch_initial_klines(self):
        ...
        self.volumes = [Decimal(str(c[5])) for c in candles]
