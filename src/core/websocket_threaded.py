...
class WebSocketThreadManager:
    ...
    def __init__(self, api_key: str, api_secret: str, symbol: str, kline_interval: str, stream_handler, user_handler, kline_queue, user_queue):
        ...
        self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=settings.MAX_WORKER_THREADS)
    ...
    def _monitor_connection(self):
        while not self._stop_event.is_set():
            ...
            with self._lock:
                if time.time() - self._last_data_received_time > settings.NO_DATA_TIMEOUT:
    ...
    def _handle_reconnection(self):
        ...
        재연결 로직을 수행합니다.
        """
        while self._reconnect_attempts < settings.MAX_RECONNECT_ATTEMPTS:
            self._reconnect_attempts += 1
            reconnect_delay = settings.INITIAL_RECONNECT_DELAY * (settings.BACKOFF_FACTOR ** (self._reconnect_attempts - 1))
            logger.info(f"Attempting reconnection... Attempt {self._reconnect_attempts}/{settings.MAX_RECONNECT_ATTEMPTS}. Waiting for {reconnect_delay:.2f} seconds.")
        ...
        logger.critical(f"Maximum reconnection attempts ({settings.MAX_RECONNECT_ATTEMPTS}) exceeded.")
        self._transition_to_state(WebSocketState.FATAL_ERROR)
