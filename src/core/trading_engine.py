...
from ..analysis.renderer import plot_candlestick_chart
...
class TradingEngine:
    ...
    def analysis_liquidity_grab(self, swing_points:List, showGrabLogger:bool = False) -> Tuple[Dict, Dict]:
        """
        Analyzes candlestick data to identify potential 'Liquidity Grab' events 
        at confirmed Swing High and Swing Low points.
        
        A Liquidity Grab (or Stop-Loss Hunt) is typically identified when the price 
        momentarily moves beyond a swing point, triggering stop-loss orders, and then 
        reverses, often with a specific candle pattern (a wick/shadow piercing the level 
        and a body closing back inside).

        The current implementation checks for a candle (i) that pierces a swing point 
        (price) and reverses, leaving a large shadow (wick) beyond the swing point.
        It specifically looks at the current candle (i) and the previous two candles (i-1, i-2) 
        and assesses the shadow ratio of the candle (i-1).

        Args:
            swing_points (List): A list of confirmed swing points, each a tuple 
                                 of (type: str, index: int, price: Decimal).
            showGrabLogger (bool): If True, logs the identified grab events.

        Returns:
            Tuple[Dict, Dict]: A tuple containing two dictionaries:
                               1. high_grab: {Liquidity_Index: Grab_Candle_Index} for Swing Highs.
                               2. low_grab: {Liquidity_Index: Grab_Candle_Index} for Swing Lows.
        """
        ...
        long_result:Tuple[OrderSignal, Decimal] = OrderSignal.NO_SIGNAL, Decimal('0')
        short_result:Tuple[OrderSignal, Decimal] = OrderSignal.NO_SIGNAL, Decimal('0')

        low_grab = {}
        high_grab = {}
        # Minimum 3 candles (i-2, i-1, i) are required for pattern analysis
        for i in range(2, len(lows)):

            # Extract candle data (i-2, i-1, i)
            open2 = opens[i-2]
            high2 = highs[i-2]
            low2 = lows[i-2]
            close2 = closes[i-2]
            ...
            # open = opens[i]  # The open price of the current candle (i) is not used in the current logic
            high = highs[i]
            low = lows[i]
            # close = closes[i] # The close price of the current candle (i) is not used in the current logic

            for swing_index in range(2, len(swing_points)):

                swing_value = swing_points[swing_index]
                prev_swing_value = swing_points[swing_index-2]

                swing_type = swing_value[0]
                idx = swing_value[1]
                price = swing_value[2]

                prev_idx = prev_swing_value[1]
                prev_price = prev_swing_value[2]

                # Candle index i must be after the swing point index idx (i > idx)
                if i > idx:
                    # ---------------------------
                    # 1. Check for LOW (Demand Side Liquidity Grab)
                    # The condition 'idx not in low_grab.keys()' ensures this swing point is checked only once.
                    # ---------------------------
                    if swing_type == "LOW" and idx not in low_grab.keys():
                        
                        # Liquidity Grab Candidate Candle: i-1
                        # Case 1: i-1 is a Bullish candle (open1 < close1)
                        if open1 < close1:
                            # Upper shadow: High1 - Close1, Lower shadow: Open1 - Low1
                            higher_shadow = high1 - close1
                            lower_shadow = open1 - low1
                            
                            # Check for specific Low Grab pattern
                            if low1 < price < open1 and low > price and low2 > price and higher_shadow < lower_shadow and price <= prev_price:
                                low_grab.update({idx : i})
                                if i == len(lows) - 1:
                                    long_result = OrderSignal.OPEN_POSITION, low1
                                if showGrabLogger:
                                    logger.info(f"LOW GRAB FOUND: Index {len(lows)-idx} at {price} grabbed by candle {len(lows)-i} at low {lows[i]}")

                            # Case 3: Simple Swing Low Break by i-1 (No specific pattern met, but low1 broke price)
                            # Mark the swing low as broken (None) to exclude it from further checks (satisfies user's requirement).
                            elif low1 < price:
                                low_grab.update({idx : None})

                        # Case 2: i-1 is a Bearish candle (open1 > close1)
                        elif open1 > close1:
                            # Upper shadow: High1 - Open1, Lower shadow: Close1 - Low1
                            higher_shadow = high1 - open1
                            lower_shadow = close1 - low1
                            
                            # Check for specific Low Grab pattern
                            if low1 < price < close1 and low > price and low2 > price and higher_shadow < lower_shadow and price <= prev_price:
                                low_grab.update({idx : i})
                                if i == len(lows) - 1:
                                    long_result = OrderSignal.OPEN_POSITION, low1
                                if showGrabLogger:
                    ...
                    # ---------------------------
                    # 2. Check for HIGH (Supply Side Liquidity Grab)
                    # The condition 'idx not in high_grab.keys()' ensures this swing point is checked only once.
                    # ---------------------------
                    elif swing_type == "HIGH" and idx not in high_grab.keys():
                        ...
                        if open1 < close1:
                            ...
                            # Check for specific High Grab pattern
                            if high1 > price > close1 and high < price and high2 < price and higher_shadow > lower_shadow and price >= prev_price:
                                high_grab.update({idx : i})
                                if i == len(lows) - 1:
                                    short_result = OrderSignal.OPEN_POSITION, high1
                                if showGrabLogger:
                                    logger.info(f"HIGH GRAB FOUND: Index {len(lows)-idx} at {price} grabbed by candle {len(lows)-i} at high {highs[i]}")

                            # Case 3: Simple Swing High Break by i-1 (No specific pattern met, but high1 broke price)
                            # Mark the swing high as broken (None) to exclude it from further checks (satisfies user's requirement).
                            elif high1 > price:
                                high_grab.update({idx : None})
                        ...
                        # Case 2: i-1 is a Bearish candle (open1 > close1)
                            ...
                            # Check for specific High Grab pattern
                            if high1 > price > open1 and high < price and high2 < price and higher_shadow > lower_shadow and price >= prev_price:
                                high_grab.update({idx : i})
                                if i == len(lows) - 1:
                                    short_result = OrderSignal.OPEN_POSITION, high1
                                if showGrabLogger:
    ...
    def identify_liquidity(self, showSwingPoint:bool = False):
        ...
        long_grab, short_grab  = self.analysis_liquidity_grab(swing_points, True)

        self.analysis_liquidity_sweep(swing_points, True)

        return long_grab, short_grab

    def analysis_liquidity_sweep(self, swing_points: List, showSweepLogger:bool = False):
        ...
        for sType, sIndex, sPrice in swing_points:
            sRIndex = len(lows) - sIndex
            # logger.info(f"{sType} {sRIndex} {sPrice}")
            ...
            if swing_low_dict != {}:
                for key in swing_low_dict:
                    ...
                    if value == None and sIndex > key and lows[key] > sPrice:
                        swing_low_dict[key] = sIndex
            ...
            if swing_high_dict != {}:
                for key in swing_high_dict:
                    ...
                    if value == None and sIndex > key and highs[key] < sPrice:
                        swing_high_dict[key] = sIndex
        ...
        #     if value == None:
        #         pass
        #         # logger.info(f"[HIGH] {len(highs)-key} {value}")
        #     else:
        #         logger.info(f"[HIGH] {len(highs)-key} {len(highs)-value}")

        # for key in swing_low_dict:
        #     value = swing_low_dict[key]
        #     if value == None:
        #         pass
        #         # logger.info(f"{len(lows)-key} {value}")
        #     else:
        #         logger.info(f"{len(lows)-key} {len(lows)-value}")
                    ...
                    if kline_data and kline_data.get('x'):
                        ...
                        if app_config.ENABLE_ORDER:
                            ...
                            if long_signal is not OrderSignal.NO_SIGNAL:
                                logger.info(f"[LONG] {long_signal} {long_sl_price}")
                            if short_signal is not OrderSignal.NO_SIGNAL:
                                logger.info(f"[SHORT] {short_signal} {short_sl_price}")
