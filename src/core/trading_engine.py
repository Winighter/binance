            ...
            if 'stream' in res and 'data' in res:
                ...
                if stream_name == f"{self.symbol.lower()}@kline_{app_config.KLINE_INTERVAL}":
                    ...
                    if kline_data.get('x'):
                        ...
                        # Candle & Volume
                        lc_conditions = (opens[-2] > closes[-2]) and \
                        ...
                        trb_condition = trend_direction == "SHORT"

                        # Commodity Trend Reactor
                        ctr_cci = self._commodity_trend_reactor(highs, lows, closes)
                        ctr_condition = ctr_cci < Decimal("-200")

                        # Long Indicator All Conditions
                        long_indicator_conditions = dtb_long and ema_condition and trb_condition and ctr_condition

                        # Long Total Conditions
                        long_condition = system_condition and lc_conditions and long_indicator_conditions
