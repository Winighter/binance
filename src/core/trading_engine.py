...
class TradingEngine:
    ...
    def analysis_liquidity_grab(self, swing_points: List, showGrabLogger:bool = False) -> Tuple:
        ...
        for key in swing_high_dict:
            value = swing_high_dict[key]
            if value:
                logger.info(f"HIGH {len(lows)-value}")
        ...
        for key in swing_low_dict:
            value = swing_low_dict[key]
            if value:
                logger.info(f"LOW {len(lows)-value}")
    ...
    def identify_liquidity(self, showSwingPoint:bool = False):
        ...
        self.analysis_internal_liquidity_fvg()

        return long_grab, short_grab

    def analysis_internal_liquidity_fvg(self):
        ...
        for i in range(2, len(lows)):
            ...
            # logger.info(f"[{ri2}-{ri1}-{ri}] {low2}-{low1}-{low}")

            # momentum = body_rate >= min_body_rate
            # if open1 < high2 and high2 < low and low < close1 and momentum:
            #     logger.info(f"[{len(lows)-i-1}] {high2} {low}")
            #     bullish_fvg.update({i-1:{'high':{high2}, 'low':{low}}})

        # for key in bullish_fvg:
        #     value = bullish_fvg[key]
        #     logger.info(f"[{len(lows)-key}] {value['high']} {value['low']}")

        #     check = {}
            # for i in range(key, len(lows)):
            #     if i > key:
            #         if key not in check.keys():
            #             if lows[i] < lows[key] < lows[i-1]:
            #                 # logger.info(f"{len(lows)-key} {len(lows)-i}")
            #                 check.update({key: None})

            #         elif key in check.keys():
            #             if check[key] == None:
            #                 if lows[i] > lows[key] > lows[i-1]:
            #                     check.update({key: i})
            #                     # logger.info(f"[{len(lows)-key}-{len(lows)-i}]")
