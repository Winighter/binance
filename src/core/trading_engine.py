...
class TradingEngine:
    ...
    def _commodity_trend_reactor(self, highs, lows, closes):
        ...
        latest_bar = analysis_results[-1]
        # if latest_bar['Square_Status'] != "중립":
        if len(analysis_results) >= 1:
            # logger.info(f"-------------------------------------")
            # logger.info(f"Bar #{latest_bar['bar_index']}:")
            # logger.info(f"  CCI: {latest_bar['CCI']:.4f}")
            # logger.info(f"  Trend Change: {latest_bar['Trend_Changed']}")
            # logger.info(f"  Trail Line: {latest_bar['Trail_Line']:.4f}")
            # logger.info(f"  Square Status: {latest_bar['Square_Status']}")
            # logger.info(f"  Trend: {'상승 (True)' if latest_bar['Trend'] else '하락 (False)' if latest_bar['Trend'] is not None else 'N/A'}")

            return latest_bar['CCI']
            ...
            if 'stream' in res and 'data' in res:
                ...
                if stream_name == f"{self.symbol.lower()}@kline_{app_config.KLINE_INTERVAL}":
                    ...
                    if kline_data.get('x'):
                        ...
                        system_condition = lows[-2] == ll == sh

                        lc_conditions = system_condition and \
                            (opens[-2] > closes[-2]) and \
                            (opens[-3] > closes[-3] or opens[-4] > closes[-4]) and \
                            (highs[-2] - opens[-2] <= closes[-2] - lows[-2]) and \
                            (volumes[-2] > volumes[-3]) and \
                            ((lows[-1] < lows[-2] and highs[-1] < highs[-2]) == False)

                        dtb_long, _ = self._dynamic_trend_bands(highs, lows, closes)

                        prev_ema = Metrics.ema(closes, 100)[-2]
                        ema_condition = prev_ema < closes[-2]
                        ...
                        long_indicator_conditions = dtb_long and ema_condition and (trend_direction == "SHORT") and (ctr_cci < Decimal("-200"))

                        long_condition = lc_conditions and long_indicator_conditions
                        long_condition = True
                        if long_condition:
                            logger.info("Entry Long Position !!!")
                            self._pullback_analyzer(opens, highs, lows, closes)
    ...
    def pullback_execute_long_trade(self, long_signal, low: Decimal, close: Decimal):
        ...
        if self.positions.long is None and long_signal == LongSignal.OPEN_POSITION:
            ...
            if len(result) == 2:
                ...
                # 예상치 못한 경우를 위한 안전 장치 (혹시라도 추가 매수 로직이 이쪽으로 온 경우 등)
                quantity = result
                sl_price = low
