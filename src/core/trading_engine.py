...
class TradingEngine:
    ...
    def create_long_position(self, quantity: Decimal, current_price: Decimal, sl_rate: Decimal = 0.04, atr_value: Decimal = None, supertrend_value: Decimal = None):
        try:
            ...
            if long_order:
                ...
                stop_loss_price = Decimal(str(self.long_entry_price * (Decimal(str(1)) - Decimal(str(sl_rate))) / Decimal(str(app_config.LEVERAGE))))

                if atr_value:
                    self.positions.long_atr_stop_loss = self.long_entry_price - (Decimal(str(atr_value)) * Decimal(str(app_config.ATR_MULTIPLIER)))

                    if supertrend_value:
                        stop_loss_price = max(supertrend_value, self.positions.long_atr_stop_loss, stop_loss_price)
                    else:
                        stop_loss_price = max(self.positions.long_atr_stop_loss, stop_loss_price)
    ...
    def create_short_position(self, quantity: Decimal, current_price: Decimal, sl_rate: Decimal = 0.04, atr_value: Decimal = None, supertrend_value: Decimal = None):
        if not quantity:
            return
        try:
            ...
            if short_order:
                ...
                stop_loss_price = Decimal(str(self.short_entry_price * (Decimal(str(1)) + Decimal(str(sl_rate))) / Decimal(str(app_config.LEVERAGE))))

                if atr_value:
                    self.positions.short_atr_stop_loss = self.short_entry_price + (Decimal(str(atr_value)) * Decimal(str(app_config.ATR_MULTIPLIER)))

                    if supertrend_value:
                        stop_loss_price = min(supertrend_value, stop_loss_price, self.positions.short_atr_stop_loss)
                    else:
                        stop_loss_price = min(stop_loss_price, self.positions.short_atr_stop_loss)
