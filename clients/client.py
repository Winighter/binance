...
class BinanceClient:
    ...
    @staticmethod
    def _safe_api_call(api_method, *args, **kwargs):
        """
        API 호출을 안전하게 처리하는 정적 헬퍼 메서드.
        """
        try:
    ...
    def get_orderbook_ticker(self):
        ...
        """
        return self._safe_api_call(self.client.futures_orderbook_ticker, symbol=self.symbol)
    ...
    def get_position_information(self):
        ...
        """
        return self._safe_api_call(self.client.futures_position_information, symbol=self.symbol)
    ...
    def change_leverage(self, leverage: int):
        ...
        """
        return self._safe_api_call(self.client.futures_change_leverage, symbol=self.symbol, leverage=leverage)

    def get_klines(self, interval: str, limit: int = app_config.DEFAULT_KLINE_LIMIT):
        """
        ...
        """
        return self._safe_api_call(self.client.futures_klines, symbol=self.symbol, interval=interval, limit=limit)
    ...
    def create_market_order(self, side: str, positionSide:str, quantity: float):
        """
        ...
        """
        return self._safe_api_call(self.client.futures_create_order, symbol=self.symbol, side=side, type='MARKET', positionSide=positionSide, quantity=quantity)
