...
class Binance:
    ...
    def long_short(self, rsi0, rsi1):

        long_condition = rsi1 < 25 and 25 < rsi0 # 

        short_condition = rsi1 > 75 and 75 > rsi0 #
            ...
            if msg['stream'] == f'{self.ls}@kline_{self.ki}':
                ...
                if closed == True:
                    ...
                    if self.Ribbon == True:
                        ...
                        ema = Indicator.ema(self.close_list, 66, None)

                        cLong = ema[0] >= ema[2] and ema[1] < ema[3] and ema[0] > ema[1]

                        cSHort = ema[0] <= ema[2] and ema[1] > ema[3] and ema[0] < ema[1]

                        Message(f"Ribbon {cLong, cSHort}")

                        ### Entry Condition ###
                        # Long
                        long_condition = (open < close) and cLong and ((open < ema[0] and ema[0] < close) or (open_1 < ema[1] and ema[1] < close_1))

                        # Short
                        short_condition = (open > close) and cSHort and ((open > ema[0] and ema[0] > close) or (open_1 > ema[1] and ema[1] > close_1))
                    ...
                    if self.rsi_mode == True:
                        rsi10 = Indicator.rsi(self.close_list, 10, None)

                        l10, s10 = self.long_short(rsi10[0],rsi10[1])

                        long_condition, short_condition = l10, s10

                        long_end = short_condition
                        short_end = long_condition
                        Message(f"{rsi10[1],rsi10[0]} {long_condition,short_condition}")
                ...
                self.close_list.insert(0, close)
                #####################################
                # Strategy.rsi_divergence(self.close_list)
                ...
                if self.balance_dict != {}:
                    ...
                    if "LONG" in self.balance_dict[self.symbol].keys():
                        ...
                        if self.symbol not in self.order_sell_dict.keys():
                            ...
                            # LONG 전부 손절 (조건)
                            if lurp <= self.risk:
                                Message("[Profit] Close Long Position")
                                self.HALF = False
                                self.orderFO(self.symbol, "SELL", lpositionAmt)
                    ...
                    # SHORT 정리
                        ...
                        if self.symbol not in self.order_buy_dict.keys():
                            ...
                            # SHORT 전부 손절 (조건)
                            if surp <= self.risk:
                                Message("[Profit] Close Short Position")
                                self.HALF = False
                                self.orderFO(self.symbol, "BUY", spositionAmt)
