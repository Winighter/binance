...
from ..shared.enums import FilterType, AssetType, MarginType
...
class BinanceSetupManager:
    ...
    def _setup_initial_state(self) -> tuple[list[List[Decimal]], int, int]:
        ...
        try:
            ...
            symbol_precision.update({'ohlc_prices':ohlc_prices})
    ...
    def _fetch_massive_klines(self):
        ...
        try:
            ...
            logger.info(f"API: Successfully fetched total {len(final_candles)+1} candles (Current included).")
    ...
    def _fetch_symbol_info(self):
        symbols_info = self.client.get_symbol_info(symbol=self.symbol)
        if not symbols_info:
    ...
    def _setup_trading_environment(self, showLog:bool = False):
        ...
        try:
            ...
            # self.client.futures_change_multi_assets_mode()
