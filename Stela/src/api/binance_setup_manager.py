...
class BinanceSetupManager:
    ...
    def __init__(self, binance_client, symbol: str, leverage: int, kline_intervals:List):
        ...
        # 1. 절대 경로를 사용하여 경로 꼬임 방지
        base_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        self.cache_dir = os.path.join(base_path, "cache_data")

        # 2. exist_ok=True 추가 (멀티스레드 대응)
    ...
    def _kline_worker(self, interval_obj, max_interval):
        ...
        if all_candles and len(all_candles) > 1:
            ...
            try:
                ...
                # 2. 경로 및 폴더 강제 확인 (에러 방지 핵심)
                # cache_path가 상대 경로라면 절대 경로로 변환하여 확실하게 만듭니다.
                abs_cache_path = os.path.abspath(cache_path)
                abs_cache_dir = os.path.dirname(abs_cache_path)
                
                # 저장 직전에 폴더가 없으면 다시 한 번 생성 (멀티스레드 충돌 방지)
                if not os.path.exists(abs_cache_dir):
                    os.makedirs(abs_cache_dir, exist_ok=True)

                # 3. 최종 저장
                np.save(abs_cache_path, structured_data)
                self.ohlc[code] = structured_data

                # logger.info(f"[{code}] 저장 성공: {abs_cache_path}")
