...
class BinanceClient:
            ...
            try:
                ...
                # C. 재시도 가능 오류 (지수 백오프 적용) 4순위
                    ...
                    if attempt < max_retries:
                        ...
                        logger.warning(f"🔄 Reconnect {attempt}/{max_retries}: {friendly_msg}. {sleep_time:.2f}초 후 다시 시도합니다.")
    ...
    def futures_trade_fees(self, symbol: Optional[str] = None) -> list[dict]:
        '''
        commissionAsset : 수수료 타입
        '''
        result = self._safe_api_call(self.client.futures_account_trades, symbol=symbol)
        for i in result:
            s = i.get('symbol')
            if s == symbol:
                fee_type = i.get('commissionAsset') # USDT, BNB
                logger.info(f"{i}")
