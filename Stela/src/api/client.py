...
class RateLimiter:
    ...
    def update_weight(self, response_headers):
        ...
        # 1. 20 이상 차이 날 때만 로그 (영수님이 확인하신 부분)
        # if abs(self.used_weight - new_weight) >= 20:
        #     logger.debug("📊 가중치 변동 감지: %d -> %d (차이: %d)", 
        #                 self.used_weight, new_weight, new_weight - self.used_weight)

        # 2. 파이썬에서 단순 변수 대입(=)은 원자적(Atomic) 연산입니다. 
        # 즉, 굳이 threading.Lock()을 걸지 않아도 데이터가 깨지지 않습니다.
        # Lock을 제거함으로써 멀티스레드 환경의 CPU 오버헤드를 완전히 없앱니다.
