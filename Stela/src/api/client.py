...
from ..shared.enums import MarginType, AssetType, Side, AlgoOrderType, PositionSide
...
class RateLimiter:
    ...
    def update_weight(self, response_headers):
        ...
        if self.used_weight != new_weight:
    ...
    def _safe_api_call(self, func, *args, **kwargs) -> Union[Dict, list, None]:
        ...
        while attempt < self.max_retries:
            ...
            try:
                ...
                # 1. 11001 에러(DNS 문제) 여부 판별하여 가상 코드 부여
                # 로그(bot.log)에서 확인된 [Errno 11001] 및 getaddrinfo 키워드 활용
                error_code = -9999 if "11001" in raw_msg or "getaddrinfo" in raw_msg else -9998

                # [Action 1] 성격 분류 및 메시지 추출 (errors.py의 ErrorManager 활용)
                exc_class = ErrorManager.get_exception_class(error_code, method_name)
                friendly_msg = ErrorManager.get_friendly_message(error_code, raw_msg)

                attempt += 1
                if attempt < self.max_retries:
                    # 앞서 수정하신 방식과 통일하여 10% 대기 시간을 적용합니다.
                    sleep_time = self.max_retry_delay * 0.1
                    # 로그도 %s 방식으로 통일하여 RAM을 아낍니다.
                    logger.warning("Network connection lost: %s. Retrying %d/%d in %.2fs...",
                       e, attempt, self.max_retries, sleep_time)
                ...
                # C. 재시도 가능 오류 4순위
                    ...
                    attempt += 1
                    if attempt < self.max_retries:
                        sleep_time = MAX_RETRY_DELAY * 0.1
                        logger.warning("🔄 Reconnect %d/%d: %s. %.2f초 후 다시 시도합니다.", 
                        attempt, self.max_retries, friendly_msg, sleep_time)
            ...
            for asset in account_balance:
                match asset.get('asset'):
                    case asset_type.value:
                        ...
                        current_usdt = Decimal(str(balance))
                        available_usdt = Decimal(str(availableBalance))

                    case AssetType.BNB.value:
                        bnb = asset.get('crossWalletBalance')
                        current_bnb = Decimal(str(bnb))

            return current_usdt, available_usdt, current_bnb 
        except Exception as e:
