...
# def send_order_buy_msg(symbol, side, stop_loss, entry, take_profit color):
#     if side == "LONG":
#         icon = '🟢'
#         side_text = "Long"
#     else:
...
def send_order_sell_msg(symbol, side, percent, amount):
    if side == "LONG":
        icon = '🟢'
        side_text = "Long"
    else:
    ...
    if percent >= 0 and amount > 0:
        color = MsgColorCode.GREEN.value
    elif percent <= 0 and amount < 0:
        color = MsgColorCode.RED.value
    else:
        color = MsgColorCode.GRAY.value

    pct_text = f" {precent_prefix}{abs(percent):.2f}%"
    amt_text = f"{amount_prefix}{abs(amount):.3f} {app_config.FUTURES_TRADING_ASSET}"
    content = f"{pct_text}{amt_text:>{28 - len(pct_text)}}"
