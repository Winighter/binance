...
class WebSocketThreadManager:
    ...
    def __init__(self, api_key: str, api_secret: str, symbol: str, kline_interval, stream_handler, user_handler, kline_queue, user_queue):
        ...
        while not self._stop_event.is_set():
            ...
            with self._lock:
                ...
                if elapsed_time > NO_DATA_TIMEOUT:
                    ...
                    logger.critical(
                        f"🚨 [WATCHDOG] {elapsed_time:.1f}초간 데이터 수신 없음! "
                        "프로세스를 강제 종료하고 PM2에 의한 클린 재시작을 수행합니다."
                    )
                    # self.stop()
                    # self._handle_reconnection()
                    # break
                    # PM2가 즉시 감지하고 다시 실행할 수 있도록 프로세스 완전 종료
                    os._exit(1)
