...
class WebSocketThreadManager:
    ...
    def __init__(self, api_key: str, api_secret: str, symbol: str, kline_intervals, kline_queue, user_queue, stop_event):
        ...
        self.symbol = symbol
        self.kline_interval = kline_intervals
    ...
    def _start_threaded_websocket_manager(self, showLog:bool = True):
        try:
            ...
            for interval in self.kline_interval:
                if interval is None:
                    continue

                interval_code = interval.code if hasattr(interval, 'code') else interval
                streams.append(f'{self.symbol.lower()}@kline_{interval_code}')
    ...
    def _handle_reconnection(self):
        ...
        while self._reconnect_attempts < MAX_RECONNECT_ATTEMPTS:
            ...
            reconnect_delay = INITIAL_RECONNECT_DELAY * self._reconnect_attempts
