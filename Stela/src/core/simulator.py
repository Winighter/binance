...
class TradeSimulator:
    ...
    def run(self, long_signals:List, short_signals:List, highs, lows, stepSize):
    ...
    def _calculate_combined_outcomes(self, combined_signals: List, highs: List[Decimal], lows: List[Decimal]) -> Dict[str, Any]:
        ...
        for ev in trade_events:
            ...
            # 2. 총 거래 가치 기반 수수료 계산
            entry_val = amount * ev["entry"]
            exit_val = amount * (ev["entry"] + ev["pnl_unit"])
            total_fee = (entry_val + exit_val) * fee_rate # 왕복 수수료

            # 3. 순수익(ROI) 계산: (수익금 - 수수료)
            roi = (amount * ev["pnl_unit"]) - total_fee
            self.sim_balance += roi # 이제 수수료가 빠진 실제 돈이 쌓입니다.

            fee_pct_impact = fee_rate * self.leverage * self.PCT_100
            net_profit_pct = ev["profit_pct"] - fee_pct_impact
            ...
            if self.show_detail:
                ...
                logger.info(
                    ...
                    f"PNL: {ev['result']:2} ({ev['profit_pct']:6.2f}%) | ROI: {roi:.4f} | "
    ...
    def get_position_quantity(self, position:PositionSide, price: Decimal, stop_loss_price: Decimal):
        ...
        if price == stop_loss_price:
                logger.warning(f"Skipping trade: Entry price and SL price are identical ({price}).")
                return 0
        try:
