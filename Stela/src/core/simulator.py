...
class TradeSimulator:
    ...
    def run(self, execute_signals:List, highs, lows, stepSize, mtf_highs, mtf_lows):
    ...
    def _calculate_combined_outcomes(self, execute_signals: List, highs: List[Decimal], lows: List[Decimal]) -> Dict[str, Any]:
        ...
        for side, index, entry, stop_loss, take_profit, check_value in sorted_signals:
            ...
            if have_position or index <= last_exit_idx:
                continue
        ...
        # logger.info(f"In Win: {poi_in_win}, In Loss: {poi_in_loss}, Out Win: {poi_out_win}, Out Loss: {poi_out_loss}, ")
