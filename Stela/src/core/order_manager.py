...
class OrderManager:
        ...
        if order_lock:
            ...
            if self.positions.long_amount == None and self.positions.long_entry_price == None:
                ...
                for id in open_id:
                    if id != None:
                        if showLog:
                            logger.info(f"{self.symbol} Long Position Not open, cancel Long Exit Order.")
            ...
            if self.positions.short_amount == None and self.positions.short_entry_price == None:
                ...
                for id in open_id:
                    if id != None:
                        if showLog:
                            logger.info(f"{self.symbol} Short Position Not open, cancel Short Exit Order.")
                        self.cancel_algo_order(self.symbol, id)
    ...
    def update_exit_order(self, ps:PositionSide, amount:Decimal, entry_price:Decimal, sl_price:Decimal, tp_price:Decimal):
        try:
            if ps == PositionSide.LONG:
                ...
                self.positions.short_entry_price = Decimal(str(entry_price))
