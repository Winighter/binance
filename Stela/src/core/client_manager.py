...
from src.config import QUEUE_MAX_SIZE
...
class ClientManager:
    ...
    @staticmethod
    def initialize_clients(symbol, kline_intervals:list, leverage, stop_event, ENABLE_SIMULATION:bool):
        try:
            ...
            setup_data = BinanceSetupManager(binance_client, symbol, leverage, kline_intervals)._setup_initial_state()
            market_data = MarketDataProcessor(binance_client, symbol, kline_intervals, setup_data)
            ...
            del setup_data
            import gc
            gc.collect()    # 즉시 메모리 해제 시도
            if ENABLE_SIMULATION:
                return (market_data, trading_engine, None, None, None)
        
            # Websocket
            kline_queue = queue.Queue(maxsize=QUEUE_MAX_SIZE)
            user_queue = queue.Queue(maxsize=QUEUE_MAX_SIZE)
