...
class MarketDataProcessor:
    def __init__(self, binance_client, symbol, kline_intervals:KlineInterval, setup_data):
        ...
        # 2. 기존 캔들 데이터 초기화
        ohlc = setup_data.get('ohlc', {})
        self.initialize_candle_data(ohlc)
    ...
    def initialize_candle_data(self, ohlc: Dict):
        """
        ...
        for interval in valid_intervals:
            ...
            if code in ohlc:
                ...
                self.candles[code] = ohlc[code]

    def update_candle_data(self, ohlc):
        try:
            interval_code = ohlc.get('i')
            if interval_code not in self.candles: return

            new_ts = int(ohlc.get('t'))
            is_closed = ohlc.get('x') # 바이낸스 x는 원래 bool 타입입니다.

            # 1. 새 데이터 준비 (튜플)
            new_entry = (
                new_ts, 
                float(ohlc['o']), float(ohlc['h']), float(ohlc['l']), float(ohlc['c'])
            )

            # 2. 로직 분기 (핵심 수정 부분)
            if is_closed:
                # [봉 마감 시] 밀어내기 수행 (기존 갯수 유지)
                current_arr = self.candles[interval_code]
                max_len = len(current_arr)
                
                # 새 배열 생성 및 데이터 시프트 (0번 버리고 1~끝을 0~끝-1로)
                updated_arr = np.empty(max_len, dtype=OHLC_DT)
                updated_arr[:-1] = current_arr[1:]
                updated_arr[-1] = new_entry # 마지막 칸에 마감 데이터 확정
                
                self.candles[interval_code] = updated_arr
                # logger.info(f"[{interval_code}] 봉 마감 완료. 크기 고정: {max_len}")
            
            else:
                # [봉 진행 중] 실시간 가격만 마지막 칸에 계속 갱신
                # 타임스탬프가 같은지 확인하여 '진행 중인 봉'인지 체크
                if self.candles[interval_code]['timestamp'][-1] == new_ts:
                    self.candles[interval_code][-1] = new_entry
