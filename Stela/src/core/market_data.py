...
class MarketDataProcessor:
    ...
    def update_balance(self, showLog:bool = False):
        try:
            self.balances.reset()
                    ...
                    for pos in positions:
                        ...
                        if amount > 0:
                            ...
                            if side == PositionSide.LONG.value:
                                ...
                                if showLog:
                                    logger.info(f"{self.symbol} [LONG-Position] amount:{amount}, entry: {entry_price:.4f}, sl_raw: {default_stop_loss}")
                                ...
                                if showLog:
                                    logger.info(f"{self.symbol} [SHORT-Position] amount:{amount}, entry: {entry_price:.4f}, sl_raw: {default_stop_loss}")
