...
class MarketDataProcessor:
    ...
    def update_balance(self, showLog:bool = True):
        try:
            self.balances.reset()
            self.balances.balance, self.balances.available_balance = self.client.futures_account_balance()
            if showLog:
                            ...
                            match (order['positionSide'], order['side']):
                                case (PositionSide.LONG.value, Side.SELL.value):
                                    match order['orderType']:
                                        case AlgoOrderType.STOP_MARKET.value:
                                            if showLog:
                                                logger.info(f"[LONG-OPEN-ORDER] Stop-Market price: {tp}, CLient ID: {order_id}")
                                        ...
                                        case AlgoOrderType.TAKE_PROFIT_MARKET.value:
                                            if showLog:
                                                logger.info(f"[LONG-OPEN-ORDER] Take-Profit price: {tp}, CLient ID: {order_id}")
                                ...
                                case (PositionSide.SHORT.value, Side.BUY.value):
                                    match order['orderType']:
                                        case AlgoOrderType.STOP_MARKET.value:
                                            if showLog:
                                                logger.info(f"[SHORT-OPEN-ORDER] Stop-Market price: {tp}, CLient ID: {order_id}")
                                            self.positions.short_stop_loss = tp
                                            self.positions.short_stop_loss_order_id = order_id

                                        case AlgoOrderType.TAKE_PROFIT_MARKET.value:
                                            if showLog:
                                                logger.info(f"[SHORT-OPEN-ORDER] Take-Profit price: {tp}, CLient ID: {order_id}")
