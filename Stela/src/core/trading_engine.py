...
class TradingEngine:
    ...
    def process_exit_levels(self, long_raw: Dict = None, short_raw: Dict = None):
        ...
        def calculate_logic(side: PositionSide, sl_raw: Decimal, entry_raw: Decimal):
            ...
            stt_adjustment = Decimal(str(self.tickSize)) * SLIPPAGE_TOLERANCE_TICKS
            ...
            if side == PositionSide.LONG:
                ...
                result_sl = result_sl + stt_adjustment
                if result_sl >= entry:
                ...
                tp_price = tp_price + stt_adjustment
                ...
                if stream_name == f"{self.symbol.lower()}@kline_{self.kline_interval}":
                    ...
                    if kline_data and kline_data.get('x'):
                        ...
                        if _long:
                            ...
                            # logger.info(f"Long Signal Index: {total_len - long_index}")
                        ...
                        if _short:
                            ...
                            # logger.info(f"Short Signal Index: {total_len - short_index}")
            ...
            match user_event:
                ...
                case UserDataEventType.ALGO_UPDATE.value:
                    ...
                    if self.symbol == symbol:
                        ...
                        # init Order Id & Price (initialize)
                        if dataX == 'FINISHED':
                            cancel_caid = str('')
                            caid = algo_data.get('caid') # Algo Order Client ID
                            quantity = algo_data.get('q') # Quantity
                            exit_type = algo_data.get('o') # 'STOP_MARKET' or 'TAKE_PROFIT_MARKET'
                            data_s = algo_data.get('S') # 'BUY' or 'SELL'
                            data_ps = algo_data.get('ps') # 'LONG' or 'SHORT'
                            ...
                            if caid:
                                if data_ps == 'LONG' and data_s == 'SELL':
                                    if exit_type == 'STOP_MARKET' and self.positions.long_stop_loss_order_id == caid:
                                        self.positions.long_stop_loss = None
                                        self.positions.long_stop_loss_order_id = None
                                        self.positions.long_default_stop_loss = None

                                    elif exit_type == 'TAKE_PROFIT_MARKET' and self.positions.long_take_profit_order_id == caid:
                                        self.positions.long_take_profit = None
                                        self.positions.long_take_profit_order_id = None
                                        self.positions.long_default_stop_loss = None

                                elif data_ps == 'SHORT' and data_s == 'BUY':
                                    if exit_type == 'STOP_MARKET' and self.positions.short_stop_loss_order_id == caid:
                                    ...
                                    elif exit_type == 'TAKE_PROFIT_MARKET' and self.positions.short_take_profit_order_id == caid:
                                        self.positions.short_take_profit = None
                                        self.positions.short_take_profit_order_id = None
                                        self.positions.short_default_stop_loss = None

                                self.order_manager.initialize_bot_state()
                    ...
                    if order_status in [OrderStatus.PARTIALLY_FILLED.value, OrderStatus.FILLED.value]:
                        ...
                        if order_status == OrderStatus.FILLED.value and q and q == z:
                            ...
                            match o.get('ps'):
                                ...
                                case PositionSide.LONG.value:
                                    # Open Long
                                    if order_side == Side.BUY.value and self.positions.long_default_stop_loss and \
                                        self.positions.long_stop_loss and self.positions.long_stop_loss_order_id and \
                                        self.positions.long_take_profit and self.positions.long_take_profit_order_id:
                                        sl_price, tp_price = self.calculate_refined_exit_prices(PositionSide.LONG, self.positions.long_default_stop_loss, exit_price)
                                ...
                                case PositionSide.SHORT.value:
                                    # Open Short
                                    if order_side == Side.SELL.value and self.positions.short_default_stop_loss and \
                                        self.positions.short_stop_loss and self.positions.short_stop_loss_order_id and \
                                        self.positions.short_take_profit and self.positions.short_take_profit_order_id:
                                        sl_price, tp_price = self.calculate_refined_exit_prices(PositionSide.SHORT, self.positions.short_default_stop_loss, exit_price)
    ...
    def calculate_refined_exit_prices(self, side: PositionSide, sl_raw: Decimal, entry_price: Decimal):
        ...
        total_fee_rate = fee_rate * 2 # 왕복 수수료
        ...
        if side == PositionSide.LONG:
            ...
            max_sl_price = entry * (Decimal('1') - actual_limit_ratio) + (entry * fee_rate * 2)
            result_sl = max(raw_sl, max_sl_price)
            result_sl = result_sl + stt_adjustment # 퇴출 시 밀림 방어
            ...
            net_loss_at_sl = (entry - result_sl) + (entry + result_sl) * fee_rate
            tp_price = (entry * (1 + fee_rate) + rr_ratio * net_loss_at_sl) / (1 - fee_rate)
            tp_price = tp_price + stt_adjustment # 목표 수익 보존

        else:  # SHORT
            # 손절가 계산
            max_sl_price = entry * (Decimal('1') + actual_limit_ratio) - (entry * fee_rate * 2)
            result_sl = min(raw_sl, max_sl_price)
            result_sl = result_sl - stt_adjustment # 퇴출 시 위로 튐 방어
    ...
    def sync_initial_exit_levels(self):
        ...
        if self.market_data.positions.long_amount:
            ...
            current_sl = self.market_data.positions.long_stop_loss
            
            if current_sl and entry_price:
                # [역산] 보정되었던 가격에서 stt를 빼서 원본 sl_raw를 추정
                recovered_sl_raw = current_sl - stt_adjustment
                self.positions.long_default_stop_loss = recovered_sl_raw # 복구된 값 박제
                
                # 실제 평단 기준으로 재보정
                sl_price, tp_price = self.calculate_refined_exit_prices(
                    PositionSide.LONG, recovered_sl_raw, entry_price
                )
                ...
                logger.info(f"[SYNC] LONG position restored successfully. SL_RAW: {recovered_sl_raw}")

        # 2. SHORT 포지션 확인
        if self.market_data.positions.short_amount:
            entry_price = self.market_data.positions.short_entry_price
            current_sl = self.market_data.positions.short_stop_loss
            
            if current_sl and entry_price:
                # [역산] SHORT는 퇴출 시 stt를 뺐었으므로, 복구할 때는 더해줌
                recovered_sl_raw = current_sl + stt_adjustment
                self.positions.short_default_stop_loss = recovered_sl_raw # 복구된 값 박제
                
                # 실제 평단 기준으로 재보정
                sl_price, tp_price = self.calculate_refined_exit_prices(
                    PositionSide.SHORT, recovered_sl_raw, entry_price
                )
                ...
                logger.info(f"[SYNC] SHORT position restored successfully. SL_RAW: {recovered_sl_raw}")