...
class TradingEngine:
    ...
    def process_trading_signals(self, showLog:bool = False):
        ...
        if showLog:
            ...
            volumes = logger_data['volumes']
            sessions = logger_data['sessions']
            f_time = format_timestamp(timestamps[-1])
            logger.info(f"{f_time, opens[-1], highs[-1], lows[-1], closes[-1], volumes[-1], sessions[-1]}")

        # for i in range(self.tl):
        #     logger.info(f"{timestamps[i], opens[i], highs[i], lows[i], closes[i], volumes[i], sessions[i]}")
        ...
        # 신호 필터 함수
        if app_config.ENABLE_SIMULATION:
            self.simulator.run(signals_with_exit, mtf_data['highs'], mtf_data['lows'], self.stepSize)
    ...
    def process_stream_data(self, res):
        ...
        try:
            if 'stream' in res and 'data' in res:
                ...
                # logger.info(f"{stream_name} {kline_data}")
                if kline_data.get('x'):
                    '''
                    HTF : ? (Identify Swing Structure + Swing Range + POIs)
                    MTF : 15m (Identify Internal Structure + POI)
                    LTF : 5m (Trade Execution for MTF Detail POIs)
                    '''
                    ...
                    # LTF
                        ...
                        if order_signal != OrderSignal.NO_SIGNAL:
                            ...
                            # else:
                            #     logger.info(f"[{side}] Balance: {self.balances.available_balance} | Entry: {entry}, Stop Loss: {stop_loss}, Take Profit: {take_profit}")
    ...
    def calculate_logic(self, side: PositionSide, entry_price: Decimal, stop_loss_price: Decimal = None, take_profit_price: Decimal = None):
        ...
        # test_leverage = 1 # Only Test
        # max_loss_limit_ratio = Decimal(str(MAX_STOP_LOSS_RATIO)) / 100
        # maximum_stop_price = entry_price * (1 - (max_loss_limit_ratio / test_leverage) if side == PositionSide.LONG else 1 + (max_loss_limit_ratio / test_leverage)) 
