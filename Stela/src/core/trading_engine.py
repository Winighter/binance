...
class TradingEngine:
    ...
    def __init__(self, binance_client, market_data, order_manager, symbol, leverage, kline_intervals, setup_data):
        ...
        self.stepSize = Decimal(str(setup_data.get('stepSize')))
        self.tickSize = Decimal(str(setup_data.get('tickSize')))
    ...
    def process_exit_levels(self, execute_signals, showLog:bool = False):
        ...
        if execute_signals:
            ...
            for position_side, signal_index, entry_price, stop_loss_price, take_profit_price, check_value in execute_signals:

                e, sl, tp = self.calculate_logic(position_side, entry_price, stop_loss_price, take_profit_price)

                if e and sl and tp:
                    return_signals.append((position_side, signal_index, e, sl, tp, check_value))
    ...
    def process_trading_signals(self, showLog:bool = False):
        ...
        # 신호 필터 함수
        if app_config.ENABLE_SIMULATION:
            self.simulator.run(signals_with_exit, ltf_data['highs'], ltf_data['lows'], self.stepSize, mtf_data['highs'], mtf_data['lows'])
            ...
            if 'stream' in res and 'data' in res:
                ...
                if kline_data.get('x'):
                    ...
                    # LTF
                        ...
                        if signal:
                            side, index, entry, stop_loss, take_profit, check_value = signal
                            if self.tl - index == 1:
    ...
    def execute_trade(self, side:PositionSide, signal: OrderSignal, entry:Decimal, stop_loss: Decimal, take_profit: Decimal):
        ...
        if side is PositionSide.LONG:
            ...
            if self.positions.short_entry_price is None and signal == OrderSignal.OPEN_POSITION:
                quantity = self.order_manager.get_position_quantity(position=PositionSide.SHORT, entry=entry, stop_loss=stop_loss)
                test_profit = (stop_loss - entry) / entry * 100
    ...
    def calculate_logic(self, side: PositionSide, entry_price, stop_loss_price, take_profit_price = None):
        ...
        maximum_total_fee = fee_rate * (entry_price + stop_loss_price)

        # 최소한의 익절가
        minimum_take_profit = entry_price + maximum_total_fee if side == PositionSide.LONG else entry_price - maximum_total_fee
        minimum_take_profit = round_tick_size(minimum_take_profit, self.tickSize)
