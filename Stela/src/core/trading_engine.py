...
class TradingEngine:
    ...
    def trading_signal(self, long_signals, short_signals):
        ...
        if long_signals and len(long_signals) > 0:
            ...
            long = [latest_long[0], latest_long[1], latest_long[2], latest_long[3]]

        if short_signals and len(short_signals) > 0:
            latest_short = max(short_signals, key=lambda x: x[0])
            short = [latest_short[0], latest_short[1], latest_short[2], latest_short[3]]
    ...
    def process_exit_levels(self, long_raw: Dict = None, short_raw: Dict = None):
        ...
        def calculate_logic(side: PositionSide, sl_raw: Decimal, entry_raw: Decimal):
            ...
            actual_limit_ratio = Decimal(str(maximum_sl_ratio)) / Decimal(str(self.leverage)) / Decimal('100')
            slippage_amount = Decimal(str(entry * self.slippage_percent))

            # 1. 먼저 손절가와 익절가를 계산합니다.
            if side == PositionSide.LONG:
                self.positions.long_default_stop_loss = sl_raw
                max_sl_price = entry * (Decimal('1') - actual_limit_ratio) + (entry * fee_rate * 2)
                result_sl = max(raw_sl, max_sl_price)
                result_sl = result_sl - slippage_amount
                if result_sl >= entry:
                    result_sl = entry - Decimal(str(self.tickSize))
                
                net_loss_at_sl = (entry - result_sl) + (entry + result_sl) * fee_rate
                tp_price = (entry * (1 + fee_rate) + rr_ratio * net_loss_at_sl) / (1 - fee_rate)
                tp_price = tp_price - slippage_amount
                
                # [필터 로직] 익절가와 진입가의 차이가 왕복 수수료율보다 작으면 탈락
                # 즉, (TP - Entry) / Entry < 0.0009 이면 실질적 마이너스
                if (tp_price - entry) / entry <= total_fee_rate:
                    return None, None

            else:  # SHORT
                self.positions.short_default_stop_loss = sl_raw
                max_sl_price = entry * (Decimal('1') + actual_limit_ratio) - (entry * fee_rate * 2)
                result_sl = min(raw_sl, max_sl_price)
                result_sl = result_sl + slippage_amount
                if result_sl <= entry:
                    result_sl = entry + Decimal(str(self.tickSize))

                net_loss_at_sl = (result_sl - entry) + (entry + result_sl) * fee_rate
                tp_price = (entry * (1 - fee_rate) - rr_ratio * net_loss_at_sl) / (1 + fee_rate)
                tp_price = tp_price + slippage_amount

                # [필터 로직] 숏 포지션 수익률 체크
                if (entry - tp_price) / entry <= total_fee_rate:
                    return None, None
        ...
        if long_raw:
            for idx, value in long_raw.items():
                ...
                f_sl, f_tp = calculate_logic(PositionSide.LONG, sl, entry)
                if f_sl and f_tp: # 수익성이 있는 신호만 추가
                    long_signals.append([idx, f_sl, entry, f_tp])

        short_signals = []
        if short_raw:
            for idx, value in short_raw.items():
                sl, entry = value[0], value[1]
                f_sl, f_tp = calculate_logic(PositionSide.SHORT, sl, entry)
                if f_sl and f_tp: # 수익성이 있는 신호만 추가
                    short_signals.append([idx, f_sl, entry, f_tp])
                    ...
                    if kline_data and kline_data.get('x'):
                        ...
                        if ENABLE_ORDER:
                            if long_signal != OrderSignal.NO_SIGNAL:
                                self.execute_trade(PositionSide.LONG, long_signal, long_stop_loss, long_take_profit)
                            if short_signal != OrderSignal.NO_SIGNAL:
                                self.execute_trade(PositionSide.SHORT, short_signal, short_stop_loss, short_take_profit)
                        else:
                            if long_signal is not OrderSignal.NO_SIGNAL:
                                logger.info(f"[LONG] {long_signal}, Stop Loss: {long_stop_loss}, Take Profit: {long_take_profit}")
                            if short_signal is not OrderSignal.NO_SIGNAL:
                                logger.info(f"[SHORT] {short_signal}, Stop Loss: {short_stop_loss}, Take Profit: {short_take_profit}")
        ...
        try:
            ...
            match user_event:
                ...
                case UserDataEventType.ALGO_UPDATE.value:
                    ...
                    aoes = algo_data.get('X') # algoOrderEventStatus

                    if algo_data.get('s') == self.symbol and aoes in [AlgoOrderEventStatus.NEW.value, AlgoOrderEventStatus.FINISHED.value]:
                ...
                case UserDataEventType.ORDER_TRADE_UPDATE.value:
                    ...
                    fee = Decimal(str(o.get('n'))) # USDT: 1.23, BNB: 0.000123
                            ...
                            match o.get('ps'):
                                ...
                                case PositionSide.LONG.value:
                                    ...
                                    if order_side == Side.BUY.value and default_stop_loss:
                                        sl_price, tp_price = self.calculate_refined_exit_prices(PositionSide.LONG, default_stop_loss, exit_price)
                                ...
                                case PositionSide.SHORT.value:
                                    ...
                                    if order_side == Side.SELL.value and default_stop_loss:
                                        sl_price, tp_price = self.calculate_refined_exit_prices(PositionSide.SHORT, default_stop_loss, exit_price)
    ...
    def execute_trade(self, side:PositionSide, signal: OrderSignal, stop_loss: Decimal, take_profit: Decimal):
        ...
        if side is PositionSide.LONG:
            ...
            if (stop_loss <= long_entry <= take_profit) == False:
                raise ValueError(f'Check Price: {stop_loss} {long_entry} {take_profit}')

            # OPEN LONG POSTION
            if self.positions.long_entry_price is None and signal == OrderSignal.OPEN_POSITION:
                quantity = self.get_position_quantity(position=PositionSide.LONG, price=long_entry, stop_loss_price=stop_loss)
                logger.info(f"[LONG SIGNAL] Amount: {quantity:.2f} | Stop Loss: {stop_loss}, Entry: {long_entry}, Take Profit: {take_profit}")
                if quantity > 0:
                    self.order_manager.create_buy_position(position=PositionSide.LONG, quantity=quantity, entry_price=long_entry, sl_price=stop_loss, tp_price=take_profit)
                else:
                    logger.warning(f"[{self.symbol}] 리스크 관리 조건(minQty/minNotional) 미달로 주문을 생성하지 않습니다.")

        elif side is PositionSide.SHORT:

            if (stop_loss >= short_entry >= take_profit) == False:
                raise ValueError(f'Check Price: {stop_loss} {short_entry} {take_profit}')

            # OPEN SHORT POSTION
            if self.positions.short_entry_price is None and signal == OrderSignal.OPEN_POSITION:
                quantity = self.get_position_quantity(position=PositionSide.SHORT, price=short_entry, stop_loss_price=stop_loss)
                logger.info(f"[SHORT SIGNAL] Amount: {quantity:.2f} | Stop Loss: {stop_loss}, Entry: {short_entry}, Take Profit: {take_profit}")
                if quantity > 0:
                    self.order_manager.create_buy_position(position=PositionSide.SHORT, quantity=quantity, entry_price=short_entry, sl_price=stop_loss, tp_price=take_profit)
                else:
                    logger.warning(f"[{self.symbol}] 리스크 관리 조건(minQty/minNotional) 미달로 주문을 생성하지 않습니다.")
    ...
    def calculate_refined_exit_prices(self, side: PositionSide, sl_raw: Decimal, entry_price: Decimal):
        ...
        actual_limit_ratio = Decimal(str(maximum_sl_ratio)) / Decimal(str(self.leverage)) / Decimal('100')
        stt_amount = Decimal(str(entry * self.slippage_percent))

        if side == PositionSide.LONG:
            # 손절가 계산 (수수료 및 슬리피지 보정)
            max_sl_price = entry * (Decimal('1') - actual_limit_ratio) + (entry * fee_rate * 2)
            result_sl = max(raw_sl, max_sl_price)
            result_sl = result_sl - stt_amount # 퇴출 시 밀림 방어
            
            if result_sl >= entry:
                result_sl = entry - Decimal(str(self.tickSize))
            
            # 익절가 계산 (수수료 및 슬리피지 보정)
            net_loss_at_sl = (entry - result_sl) + (entry + result_sl) * fee_rate
            tp_price = (entry * (1 + fee_rate) + rr_ratio * net_loss_at_sl) / (1 - fee_rate)
            tp_price = tp_price - stt_amount # 목표 수익 보존

        else:  # SHORT
            # 손절가 계산
            max_sl_price = entry * (Decimal('1') + actual_limit_ratio) - (entry * fee_rate * 2)
            result_sl = min(raw_sl, max_sl_price)
            result_sl = result_sl + stt_amount # 퇴출 시 위로 튐 방어
    ...
    def sync_initial_exit_levels(self):
        ...
        if self.market_data.positions.long_amount:
            ...
            if entry_price and default_stop_loss:
                ...
                sl_price, tp_price = self.calculate_refined_exit_prices(
                    PositionSide.LONG, default_stop_loss, entry_price
                )
                ...
                logger.info(f"[SYNC] LONG position restored successfully. SL_RAW: {default_stop_loss}, sl_price: {sl_price}, Entry: {entry_price}, tp_price: {tp_price}")
        ...
        if self.market_data.positions.short_amount:
            ...
            if entry_price and default_stop_loss:
                # 실제 평단 기준으로 재보정
                sl_price, tp_price = self.calculate_refined_exit_prices(
                    PositionSide.SHORT, default_stop_loss, entry_price
                )
                self.order_manager.update_exit_order(
