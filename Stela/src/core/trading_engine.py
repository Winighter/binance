...
class TradingEngine:
    ...
    def process_exit_levels(self, long_raw: Dict = None, short_raw: Dict = None):
        ...
        if long_raw:
            for idx, value in long_raw.items():
                ...
                if df_sl and f_sl and f_tp: # 수익성이 있는 신호만 추가
                    # logger.info(f"LONG {total_len-idx, f_sl, entry, f_tp}")
        ...
        if short_raw:
            for idx, value in short_raw.items():
                ...
                if df_sl and f_sl and f_tp: # 수익성이 있는 신호만 추가
                    # logger.info(f"SHORT {total_len-idx, f_sl, entry, f_tp}")
                            ...
                            match o.get('ps'):
                                ...
                                case PositionSide.LONG.value:
                                    ...
                                    if order_side == Side.BUY.value and default_stop_loss:
                                        _, sl_price, tp_price = self.calculate_logic(side=PositionSide.LONG, sl_raw=default_stop_loss, entry_price=exit_price)
                                        if sl_price and tp_price and ((sl_price != self.positions.long_stop_loss) or (tp_price != self.positions.long_take_profit)):
                                            self.order_manager.update_exit_order(ps=PositionSide.LONG, amount=q, entry_price=exit_price, sl_price=sl_price, tp_price=tp_price,showLog=True)
                                ...
                                case PositionSide.SHORT.value:
                                    ...
                                    if order_side == Side.SELL.value and default_stop_loss:
                                        _, sl_price, tp_price = self.calculate_logic(side=PositionSide.SHORT, sl_raw=default_stop_loss, entry_price=exit_price)
                                        if sl_price and tp_price and ((sl_price != self.positions.short_stop_loss) or (tp_price != self.positions.short_take_profit)):
                                            self.order_manager.update_exit_order(ps=PositionSide.SHORT, amount=q, entry_price=exit_price, sl_price=sl_price, tp_price=tp_price,showLog=True)
    ...
    def calculate_logic(self, side: PositionSide, sl_raw: Decimal, entry_raw: Decimal):
    ...
    def sync_initial_exit_levels(self):
        ...
        if self.market_data.positions.long_amount:
            ...
            if entry_price and default_stop_loss:
                # 실제 평단 기준으로 재보정
                _, sl_price, tp_price = self.calculate_logic(
                    side=PositionSide.LONG, sl_raw=default_stop_loss, entry_price=entry_price
                )
                if sl_price and tp_price:
                    logger.info(f"Long {sl_price} {entry_price} {tp_price}")
        ...
        if self.market_data.positions.short_amount:
            ...
            if entry_price and default_stop_loss:
                ...
                if sl_price and tp_price:
                    logger.info(f"Short {sl_price} {entry_price} {tp_price}")
                    self.order_manager.update_exit_order(
