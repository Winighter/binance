...
class MarketMechanics(BaseStrategy):
    ...
    def analyze(self):
        ...
        htf_sessions = self.htf_sessions
        ...
        mtf_sessions = self.mtf_sessions
        ...
        ltf_sessions = self.ltf_sessions
        ...
        # 2. POI
        POIs = self.identify_points_of_interest(swing_range, mtf_highs, mtf_lows, mtf_closes, False)

        signals = self.analyze_signal(ltf_opens, ltf_highs, ltf_lows, ltf_closes,  POIs, False)

        return signals

    def identify_points_of_interest(self, swing_range, highs, lows, closes, showLog:bool = False, lookback:int = 2):
        ...
        # 모든 기본 최대한의 POI
        market_structure = MarketMechanics.identify_market_structure(highs, lows, closes, 1)
        ...
        #     for i in reversed(range(0, poi_index - 1)):

        #         high = highs[i]
        #         low = lows[i]

        #         if (poi_side == 'DEMAND' and low < poi_low) or (poi_side == 'SUPPLY' and high > poi_high):
        #             data = (poi_side, i, poi_index)
        #             if data not in lq_poi2:
        #                 lq_poi2.append(data)
        #             break
        
        # for poi_side, invalid_index, poi_index in lq_poi2:

        #     for side, index, price, bos_index in reversed(market_structure):

        #         is_demand = poi_side == 'DEMAND' and side == 'LOW'
        #         is_supply = poi_side == 'SUPPLY' and side == 'HIGH'
        ...
        #             if invalid_index < index < poi_index:
        #                 logger.info(f"[{poi_side}] {tl-index} {tl-poi_index}")
        #                 # break
        ...
        #         logger.info(f"{side} {tl-index} {tl-bos_index}")
            ...
            #     if not lq_check and low < price < high:
            #         lq_check = side
            ...
            #             poi_open = self.mtf_opens[poi_index]
            #             poi_close = closes[poi_index]

            #             next_poi_open = self.mtf_opens[poi_index + 1]
            #             next_poi_close = closes[poi_index + 1]

            #             lq_demand = poi_side == 'DEMAND' and lq_check == 'LOW' and ((poi_open < poi_close) or (next_poi_open < next_poi_close))
            #             lq_supply = poi_side == 'SUPPLY' and lq_check == 'HIGH' and ((poi_open > poi_close) or (next_poi_open > next_poi_close))

            #             if i <= poi_index:

            #                 if (lq_demand or lq_supply):

            #                     if (poi_side, poi_index) not in lq_poi:
                                    
            #                         lq_poi.append((poi_side, poi_index))

            #                     for_check = True
            #                     break

            #                 else:
            #                     logger.info(f"FAILED {poi_side} {tl-i} {tl-poi_index}")

            #     if for_check:
            #         break

        # lq_poi = sorted(lq_poi, key=lambda x: x[1])

        # #######################################################################################################################

        # MTF CONDITION (Mitigate)
        for poi_side, poi_index in lq_poi:
        ...
        # Sorted
        POIs2 = sorted(POIs2, key=lambda x: x[1])

        if showLog:
            for poi_side, poi_index, poi_miti_index, poi_invalid_idx, pd_price in POIs2:
                poi_miti_index = [tl - i for i in poi_miti_index]
                poi_miti_index = sorted(poi_miti_index, reverse=True)
                logger.info(f"[{poi_side}] {tl-poi_index} {poi_miti_index}")
    ...
    def analyze_signal(self, opens, highs, lows, closes, POIs:List, showLog:bool = False, lookback:int = 2) -> List[Tuple]:
        ...
        for poi_side, poi_index, poi_mitigate_values, poi_invalid_idx, pd_price in POIs:

            # poi_mitigate_value = int(poi_mitigate_values[0]) # Last (mitigated but recent)
            poi_mitigate_value = int(poi_mitigate_values[-1]) # First (Unmitigate)

            # logger.info(f"[TEST] {self.mtf_tl-poi_mitigate_value, self.mtf_timestamps[poi_mitigate_value]} | {self.ltf_timestamps}")
            poi_mitigated_ltf_indexes = MarketMechanics.match_sub_candles(self.mtf_timestamps[poi_mitigate_value], self.ltf_timestamps, 12)
            poi_mitigated_ltf_index = poi_mitigated_ltf_indexes[-1]
            ...
            for i in range(poi_mitigated_ltf_index + 1, tl):
                high = highs[i]
                low = lows[i]
                close = closes[i]
