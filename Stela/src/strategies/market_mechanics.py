...
class MarketMechanics(BaseStrategy):
    ...
    def analyze(self):
        ...
        signals = self.analyze_signal(ltf_opens, ltf_highs, ltf_lows, ltf_closes, POIs, True)
    ...
    def identify_points_of_interest(self, swing_range, highs, lows, closes, showLog:bool = False, lookback:int = 2):
        ...
        for poi_side, poi_index in ms_poi:
            ...
            miti_poi.append((poi_side, poi_index, mitigation_index))

        POIs = []

        # Depth Filter (Invalid POI)
        for side, index, miti_value in miti_poi:
            ...
            for i in range(index + lookback + 1, tl):
                ...
                if (side == 'DEMAND' and lows[index] > low) or \
                    (side == 'SUPPLY' and highs[index] < high):
                    invalid_index = i
                    break

            if not miti_value:
                POIs.append((side, index, miti_value, invalid_index))

            if miti_value and index < miti_value < invalid_index:
                POIs.append((side, index, miti_value, invalid_index))
        ...
        if showLog:
            for poi_side, poi_index, poi_miti_index, poi_invalid_idx in POIs:
                ...
                if poi_miti_index:
                    poi_miti_index = tl - poi_miti_index

                logger.info(f"[{poi_side}] {tl-poi_index} {poi_miti_index} {tl-poi_invalid_idx}")

        return POIs

    def analyze_liquidity_sweep(self, highs, lows, lookback = 1):

        tl = len(lows)
        result = []
        swing_point = self.identify_swing_points(highs, lows, lookback=lookback)
    ...
    def analyze_signal(self, opens, highs, lows, closes, POIs:List, showLog:bool = False, lookback:int = 2) -> List[Tuple]:
        ...
        for poi_side, poi_index, poi_mitigate_value, poi_invalid_idx in POIs:
            ...
            if poi_mitigate_value:
                ...
                for lq_side, av_lq, sweep_lq in lq_sweep_data:
                    ...
                    if poi_mitigated_ltf_index <= sweep_lq:
                        ...
                        if is_session:
                            ...
                            if (poi_side == 'DEMAND' and lq_side == 'LOW'):
                                long_data = (PositionSide.LONG, sweep_lq, closes[sweep_lq], poi_low, None)
                                #logger.info(f"{poi_side} {self.mtf_tl-poi_index}-{self.mtf_tl-poi_invalid_idx} | {lq_side} {tl-av_lq} {tl-sweep_lq}-{tl-invalid_ltf_idx}")
                                execute_signals.append(long_data)
                                break

                            if (poi_side == 'SUPPLY' and lq_side == 'HIGH'):
                                short_data = (PositionSide.SHORT, sweep_lq, closes[sweep_lq], poi_high, None)
                                #logger.info(f"{poi_side} {self.mtf_tl-poi_index}-{self.mtf_tl-poi_invalid_idx} | {lq_side} {tl-av_lq} {tl-sweep_lq}-{tl-invalid_ltf_idx}")
        ...
        for side, index, entry, stop_loss, take_profit in execute_signals:
            ...
            grouped_results[(side, index)].append(stop_loss)

        four = [(key[0], key[1], values) for key, values in grouped_results.items()]

        for side, signal_index, stop_loss_values in four:
            # 1.
            ...
            execute_signals2.append((side, signal_index, closes[signal_index], new_stop_loss, None))

        if showLog:
            # logger.info(f"{(len(execute_signals2) / self.total_days):.2f}") # 일 평균 거래 신호 횟수

            for side, index, entry, stop_loss, take_profit in execute_signals2:
