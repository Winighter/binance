...
class Binance:
    ...
    def __init__(self):
        ...
        if isinstance(self.kline_interval, KlineInterval):
            interval_obj = self.kline_interval
        else:
            interval_obj = KlineInterval.get_by_code(self.kline_interval)
        if interval_obj is None:
            raise ValueError(f"Invalid interval in settings: {self.kline_interval}")
