class Indicators():
    ...
    def supertrend_pine_style(_high: list, _low: list, _close: list, _atr_length: int = 14, _multiplier: float = 4.0, _array: int = 0):
        ...
        time_ordered_high = list(reversed(_high))
        time_ordered_low = list(reversed(_low))
        time_ordered_close = list(reversed(_close))
        time_ordered_atr = list(reversed(atr_values))  # <-- 이 리스트가 더 짧음
